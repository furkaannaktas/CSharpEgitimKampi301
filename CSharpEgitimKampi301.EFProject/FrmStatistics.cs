...
namespace CSharpEgitimKampi301.EFProject
{
    public partial class FrmStatistics : Form
    {
        ...
        lblAvgLocationPrice.Text = string.Format("{0:0.00}  ₺", db.Location.Average(x => x.Price));
            ...
            var guideIdByNameFurkanAktas = db.Guide.Where(x => x.GuideName == "Furkan" && x.GuideSurname == "Aktaş").Select(y => y.GuideId).FirstOrDefault(); 
            lblFurkanAktasLocationCount.Text = db.Location.Where(x => x.GuideId == guideIdByNameFurkanAktas).Count().ToString();    
        }
    }
}
